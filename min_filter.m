function final = min_filter(image)
    ...
    [rows, cols] = size(padded);
        for i = strips + 1 : rows - strips
            for j = strips + 1 : cols - strips
                ...
                clone(i - strips, j - strips) = min(reshape(window, f_size * f_size, 1));
            end
        end
...
function padded_image = pad(image, layers)
    ...
    padded_image = image;
    for i = 1:layers
        padded_image = cat(1, padded_image, uint8(255 + zeros(1, cols)));
        padded_image = cat(1, uint8(255 + zeros(1, cols)), padded_image);
        padded_image = cat(2, padded_image, uint8(255 + zeros(rows  + 2, 1)));
        padded_image = cat(2, uint8(255 + zeros(rows + 2, 1)), padded_image);
        rows = rows + 2;
        cols = cols + 2;
    end
end